...
for idx, entry in enumerate(data):
    if not entry.get("used", False):  # Only process data where used is False
        ...
        while retry_count < max_retries:
            try:
                ...
                if answer:  # Make sure the returned content is not empty
                    ...
                    # After processing 25 JDs, the tag set is cleared.
                    if batch_count % 25 == 0:
                        print("\n--- Processed 25 JDs, cleared tag set ---\n")
                        label = ""  # Clear label set
                ...
                print(f"API returned an error ({completed_tasks + 1}th data, retried {retry_count + 1} times):", e)
        ...
        avg_time_per_task = elapsed_time / completed_tasks  # Average time spent on each piece of data
        remaining_time = avg_time_per_task * (total_tasks - completed_tasks)  # Estimated time remaining
